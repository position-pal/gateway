 ...
 * @param checkCondition the condition to meet
 * @param timeout the maximum duration to wait for the condition to be met in ms. Default is 60 seconds.
 * @param interval the interval between each check in ms. Default is 1 second.
 ...
 */
async function eventually(checkCondition, timeout = 60_000, interval = 1_000) {
  await waitFor(async () => checkCondition(), { timeout: timeout, interval: interval });
}
