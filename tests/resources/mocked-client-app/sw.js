...
(function (self) {
  ...
  const decodeConfig = () => JSON.parse(self.atob(new URL(location).searchParams.get("firebaseConfig")));

  self.addEventListener("install", () => self.skipWaiting());

  self.addEventListener("notificationclick", (event) => {
    const { notification } = event;
    const { data: { actionUrl } } = notification;
  ...
  self.addEventListener("push", function (event) {
    messaging.onBackgroundMessage((payload) => {
      log("Notification onBackgroundMessage", "Creating Notification To Show", payload);
      const { data: { title, body, actionUrl, icon, image } } = payload;
